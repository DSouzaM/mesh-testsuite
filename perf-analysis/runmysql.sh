...
if [ $malloclib=="malloc" ]
then
	nohup perf stat -e dTLB-load-misses,iTLB-load-misses -x' ' -o $tmpfile mysqld_safe &
else
	nohup perf stat -e dTLB-load-misses,iTLB-load-misses -x' ' -o $tmpfile mysqld_safe --malloc-lib=$malloclib &
fi
